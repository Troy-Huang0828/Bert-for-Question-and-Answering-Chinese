...
# def transform_data_join_predict(input_file, output_file, swag_file):
#     with open(input_file, 'r', encoding='utf-8') as private_file:
#         data = []
#         qas = []
#         private_data = json.load(private_file)
#         answer_dict = {'text': "None", 'answer_start': 0}
#         swag_dictionary = load_swag_prediction(swag_file)
#         for i in tqdm(range(len(private_data))):
#             question = private_data[i]['question']
#             ids = private_data[i]['id']
#             dictionary_2 = {}
...
def run_evaluate(args, model, eval_features, device, eval_examples, tokenizer, best_dev_score=None):
    ...
    if best_dev_score is not None and dev_score < best_dev_score and Train:
        write_to_file_but_not_the_best(predict)
...
def main():
    ...
    parser.add_argument("--val_file", default='./dataset/public.json', type=Path,
                        help="SQuAD json for predictions. E.g., dev-v1.1.json or test-v1.1.json")
    ...
    #####data process###################
    if args.do_train:
        transport_json_to_csv('dataset/train.json', 'data/train.csv', args.context_file,Train=True)
        transform_data_to_squad('dataset/train.json', 'data/squad_train.json', args.context_file,Train=True)

    if args.do_predict:
        transport_json_to_csv(args.val_file, 'data/val.csv', args.context_file,Train=False)
        import run_swag
        run_swag.main()
        transform_data_to_squad(args.val_file, 'data/squad_public.json',args.context_file,Train=False)
    ...
    if args.do_predict:
        ...
        if os.path.exists(os.path.join(args.processed_data, 'processed_dev.pkl')):
            ...
            eval_features = convert_examples_to_features(
                ...
                max_query_length=args.max_query_length,
                is_training=False)
            eval_path = os.path.join(args.processed_data, 'processed_private.pkl')
